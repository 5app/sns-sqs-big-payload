...
export class SqsConsumer {
	...
	private handleBatch?: (
		messagesWithPayload: SqsMessage[]
	...
	constructor(options: SqsConsumerOptions) {
		...
		this.messageVisabilityTimeout =
			options.messageVisabilityTimeout ?? 20 * 60; // 20 minutes
		this.messageVisabilityInterval =
			(options.messageVisabilityInterval ?? 5 * 60) * 1000; // 5 minutes
	}
			...
			try {
				...
				if (this.isConnError(err)) {
					...
					await new Promise(resolve =>
						setTimeout(resolve, this.connErrorTimeout)
					);
				} else {
	...
	private async handleSqsResponse(
		result: ReceiveMessageCommandOutput
	): Promise<void> {
		if (result && result.Messages) {
			if (this.handleBatch) {
				...
				await Promise.all(
					result.Messages.map(message => this.processMsg(message))
				);
			}
		}
	}
	...
	private async processBatch(messages: Message[]) {
		try {
			...
			const messagesToDelete =
				await this.handleBatch(messagesWithPayload);
			...
			this.events.emit(SqsConsumerEvents.processingError, {
				err,
				messages,
			});
		}
	...
	private async getMessagePayload(
		...
		try {
			const s3Object: SqsExtendedPayloadMeta | PayloadMeta =
				JSON.parse(messageBody);
			if (
				this.extendedLibraryCompatibility &&
				attributes &&
				attributes[SQS_LARGE_PAYLOAD_SIZE_ATTRIBUTE]
			) {
			...
			if (s3PayloadMeta) {
				try {
					...
					return {
						rawPayload: await s3Response.Body.transformToString(),
						s3PayloadMeta,
					};
				} catch (err) {
